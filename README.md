...
- **Parametric node graph** - primitives, transforms, arrays, mirrors, materials, UV projection, and more flow through a typed node graph (33 node types across 6 categories). Change a parameter upstream and everything downstream recooks.
...
- **Node-based parametric modeling** - a non-destructive node graph with 33 node types: primitives (box, sphere, cylinder, cone, plane, torus, torus knot), modifiers (transform, array, mirror, merge, subdivide, delete, compute normals, UV project), materials, lights, imports, and utility nodes (null, switch, bounds, note).
...
Solarxy is a Rust workspace of 11 members: the root GUI binary plus 10 crates spanning the desktop GUI, the CLI, and the web shell, alongside the `web/` React frontend.
...
| [`solarxy-graph`](crates/solarxy-graph/) | The headless studio core: node-graph document, topology, cook engine, node registry (33 node types + typed-port coercion), undo, review, and the `Engine` facade (Command in, EventBatch out). No wgpu, no winit. |
