...
pub fn view_mode() -> Text<'static> {
    Text::from(vec![
        ...
        blank(),
        prose("Real-time 3D model viewer with Cook-Torrance PBR rendering, shadow mapping,"),
        prose("normal mapping, IBL (diffuse + specular), SSAO, bloom, and a 3-light system."),
        blank(),
        ...
        shortcut_line("Shift+A", "Toggle SSAO (screen-space ambient occlusion)"),
        ...
        blank(),
        blank(),
        section_header("DRAG & DROP"),
        blank(),
        prose("Drop a model file onto the viewer window to load it."),
        prose("Drop an .hdr or .exr file to use it as an IBL environment map."),
        blank(),
        blank(),
...
pub fn preferences() -> Text<'static> {
    Text::from(vec![
        ...
        settings_row("ssao_enabled", "true"),
