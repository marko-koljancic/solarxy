...
pub fn view_mode() -> Text<'static> {
    Text::from(vec![
        ...
        prose("Dropping a new file mid-session replaces the currently loaded model."),
        ...
        blank(),
        blank(),
        section_header("HEADS-UP DISPLAY"),
        blank(),
        prose("The HUD overlays real-time information on the viewer window."),
        blank(),
        shortcut_line(
            "Top-left",
            "Model filename, mesh count, polygon/triangle/vertex",
        ),
        prose("              counts, bounding box dimensions (when active),"),
        prose("              frame time and FPS."),
        blank(),
        shortcut_line(
            "Top-right",
            "Non-default render states: view mode, projection,",
        ),
        prose("              normals, background, wireframe weight, grid,"),
        prose("              lights lock, axes, bounds, IBL mode, SSAO,"),
        prose("              tone mapping."),
        blank(),
        shortcut_line("Bottom", "Keyboard hints bar (toggle with ?)."),
        blank(),
        prose("Toast notifications appear briefly to confirm actions (green for"),
        prose("success, red for errors)."),
        blank(),
    ])
}
...
pub fn analyze_mode() -> Text<'static> {
    Text::from(vec![
        ...
        blank(),
        blank(),
        section_header("VALIDATION CHECKS"),
        blank(),
        prose("The analyzer runs seven checks on each model:"),
        blank(),
        shortcut_line(
            "Normal mismatch",
            "Normal count does not match vertex count",
        ),
        shortcut_line("UV mismatch", "UV count does not match vertex count"),
        shortcut_line("Missing UVs", "No texture coordinates (OBJ/glTF only)"),
        shortcut_line("Non-triangulated", "Index count not divisible by 3"),
        shortcut_line("Empty indices", "Empty index buffer"),
        shortcut_line("Bad material ref", "Material ID out of range"),
        shortcut_line("Missing texture", "Texture file not found (OBJ only)"),
        blank(),
        prose("STL and PLY files do not trigger the Missing UVs check."),
        blank(),
    ])
}
...
pub fn formats() -> Text<'static> {
    Text::from(vec![
        blank(),
        section_header("SUPPORTED FORMATS"),
        blank(),
        shortcut_line("OBJ  .obj", "Meshes, materials (.mtl), textures, UVs"),
        shortcut_line("STL  .stl", "Geometry only \u{2014} no materials, no UVs"),
        shortcut_line("PLY  .ply", "Flexible attributes; optional normals and UVs"),
        shortcut_line(
            "glTF .gltf/.glb",
        ...
        blank(),
        blank(),
        section_header("FORMAT NOTES"),
        blank(),
        prose("Drag a supported file onto the viewer window to load it at any time."),
        prose("The -m / --model flag accepts any of the extensions above."),
        blank(),
        shortcut_line("OBJ", "Most complete: full material pipeline with .mtl"),
        prose("              sidecar, texture paths, UV mapping. PBR via"),
        prose("              Pr (roughness) and Pm (metallic) extensions."),
        blank(),
        shortcut_line("STL", "Geometry only. A flat clay material is applied"),
        prose("              automatically. No UV debug overlay is available"),
        prose("              (no UV data)."),
        blank(),
        shortcut_line("PLY", "Attribute support varies by file. Normals and UVs"),
        prose("              are used when present; missing attributes fall"),
        prose("              back gracefully. Companion texture auto-detection"),
        prose("              searches for matching images."),
        blank(),
        shortcut_line("glTF", "Preferred format for PBR content. Full metallic-"),
        prose("              roughness workflow, normal maps, occlusion,"),
        prose("              emissive. Embedded textures (GLB) are supported."),
        blank(),
    ])
}
