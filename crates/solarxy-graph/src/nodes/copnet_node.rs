//! The `copnet` root container: opens an
//! image network (`ContextKind::Cop`) whose display node publishes the
...
pub fn descriptor() -> NodeTypeDescriptor {
    NodeTypeDescriptor {
        ...
        opens: Some(ContextKind::Cop),
        inputs: vec![],
        outputs: vec![],
        params: params_with("Cop", vec![]),
        ...
        doc: "A container you dive into to build an image procedurally: \
              ...
              image.\n\n\
              Drop one at the root next to your `sopnet` and `matnet` nodes, \
              ...
              into the scene delta. A COP network nothing refers to still cooks \
              and still shows nothing.",
        search_aliases: &["texture", "image network"],
        glyph: "copnet",
