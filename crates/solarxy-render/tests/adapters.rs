...
fn a_model_file_cooks_into_geometry_through_the_synthesized_document() {
    ...
    let loaded = input::load(&model, None, &mut |_| {}).expect("a model file loads");
...
fn both_inputs_produce_a_cooked_engine_of_the_same_kind() {
    ...
    let reopened = input::load(&scene, None, &mut |_| {}).expect("the scene file loads");
    assert!(
...
fn an_unsupported_file_is_refused_before_anything_is_read_into_the_engine() {
    ...
    let Err(err) = input::load(&odd, None, &mut |_| {}) else {
...
fn a_missing_input_is_its_own_failure() {
    let Err(err) = input::load(Path::new("does-not-exist.slxy"), None, &mut |_| {}) else {
