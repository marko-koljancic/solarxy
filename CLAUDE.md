...
- `gui/` - **split along panel boundaries**, so a reader looking for a panel finds a module:
  - `panels/` - one module per dock tab. The tree and the canvas share one graph
    ...
    overview inset, the zoom readout and the eighteen-pixel snap, `radial` the hover ring, six operations at fixed directions with the ones that do not apply
...
All twelve tabs (Viewport, Sidebar, Review Panel, Material Inspector, Properties, Tree, Nodes, Assets, Asset Preview, Texture, Attributes, Text) live in a single `egui_dock::DockState<SolarxyTab>` owned by `EguiRenderer`; the enum has a thirteenth variant, `Retired`, that no tab is ever created with (see below). `solarxy_core::preferences::DockPrefs` holds two `Option<String>` JSON blobs that serialize that state via `egui_dock`'s `serde` feature (workspace dep `egui_dock = "0.18"` with `features = ["serde"]`):
