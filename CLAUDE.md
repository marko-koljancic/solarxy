...
| `solarxy-graph` | The headless studio core: document/topology/cook engine/registry (33 registered node types as of the Phase 15 modeling wave: the 23 MVP nodes plus the Tier-2 material/texture/modeling additions from phases 13 to 15, all behind the typed-port coercion matrix + declarative param schemas)/undo/review, and the `Engine` facade (`Command` in, `EventBatch` out, budgeted resumable cook, `take_scene_delta`, `pick`, JSON `save_document`/`load_document`). Mirror-and-command model. No wgpu, no winit. |
