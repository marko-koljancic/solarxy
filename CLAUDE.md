...
**14-crate workspace + the `web/` frontend** (the web-expansion milestone added `solarxy-kernel`, `solarxy-graph`, `solarxy-scenefile`, `solarxy-web`, and `web/`; 0.9.0 added `solarxy-bvh`):
...
| `solarxy-cli` | clap `Args`, the analyze surface (`tui/`, a tiled terminal workspace: capability tiers, file-based themes, a split tree, one keymap table, a braille rasteriser, ten panel types), analyzer (`calc/analyze.rs`), its own `[[bin]]` at `src/bin/solarxy-cli.rs` which assembles the surface itself. `--mode view` spawns the `solarxy` GUI binary as a subprocess. |
