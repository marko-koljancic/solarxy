...
- `gui/` — egui integration, one responsibility per file:
  ...
  - `pane_toolbar.rs` — the slim per-pane toolbar strip (`PANE_TOOLBAR_HEIGHT` = 22 px) atop every viewport pane: Pane Mode / Shading / Inspection / Override / Projection / Show / Background combos writing **directly** into `&mut [PaneDisplaySettings; 4]` via `PaneToolbarData` (no `GuiSnapshot` round-trip); `⋯` overflow below ~360 px.
