...
- `gui/` - **split along panel boundaries**, so a reader looking for a panel finds a module:
  - `panels/` - one module per dock tab: `sidebar`, `outliner`, `node_tree`, `properties`,
    `console`, `material_inspector`, and `review/` (panel, overlay, popup, and the `visuals` the
    first two share, because marker colour and panel chip colour are the reader's first
    correlation cue).
